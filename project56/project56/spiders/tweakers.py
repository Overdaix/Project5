...
class TweakersSpider(CrawlSpider):
    name = "tweakers" # _name of the spider, to be used when crawling
    ...
    def parse_item(self, response):
        ...
        for product in products:
            ...
            if 'Geheugen intern' in category:
                ...
                item['_category_id'] = 1
                item['_name'] = ''.join(product.xpath('.//tr[contains(td[1], "Uitvoering")]/td[2]/a/text()').extract())
                item['_brand'] = ''.join(product.xpath('.//tr[contains(td[1], "Merk")]/td[2]/a/text()').extract())
                ...
                item["Picture"] = ''.join(product.xpath('.//tr[contains(td[1], "Afbeeldingen")]/td[2]/a[1]/img/@src').extract())
                items.append(item)
                ...
                item['Ean'] = ''.join(product.xpath('.//tr[contains(td[1], "EAN")]/td[2]/text()').extract())
                item['Sku'] = ''.join(product.xpath('.//tr[contains(td[1], "SKU")]/td[2]/text()').extract())
                item["Picture"] = ''.join(product.xpath('.//tr[contains(td[1], "Afbeeldingen")]/td[2]/a[1]/img[1]/@src').extract())
                items.append(item)
                ...
                item['_category_id'] = 8
                item["Picture"] = ''.join(product.xpath('.//tr[contains(td[1], "Afbeeldingen")]/td[2]/a[1]/img[1]/@src').extract())
                item['_name'] = ''.join(product.xpath('.//tr[contains(td[1], "Product")]/td[2]/a/text()').extract())
                item['_category_id'] = ''.join(product.xpath('.//tr[contains(td[1], "Categorie")]/td[2]/a/text()').extract())
                item['_brand'] = ''.join(product.xpath('.//tr[contains(td[1], "Merk")]/td[2]/text()').extract())
                item['Sku'] = ''.join(product.xpath('.//tr[contains(td[1], "SKU")]/td[2]/text()').extract())
                item['Ean'] = ''.join(product.xpath('.//tr[contains(td[1], "EAN")]/td[2]/text()').extract())
                item['Socket'] = ''.join(product.xpath('.//tr[contains(td[1], "Socket")]/td[2]/text()').extract())
                item['Speed'] = ''.join(product.xpath('.//tr[contains(td[1], "Snelheid")]/td[2]/text()').extract())
                item['CoreAmount'] = ''.join(product.xpath('.//tr[contains(td[1], "Aantal cores")]/td[2]/text()').extract())
                item['Specs'] = ''.join(product.xpath('.//tr[contains(td[1], "Geheugen Specificatie")]/td[2]/text()').extract())
                items.append(item)
